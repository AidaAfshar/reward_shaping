def safe_long_dist(v1, v2, c=None):
    if same_lane(v1, v2, c) and behind(v1, v2, c):      # TODO: double check this
        tau = c["REACTION_TIME"]
        acc_max = c["ACCEL_MAX"]
        ...
        return c["LON_DIST_NORMALIZER"] * d_safe        # TODO: this is cheating!
    else:
        return -float('inf')


def safe_lat_dist(v1, v2, c=None):
    ...
    return c["LAT_DIST_NORMALIZER"] * d_safe           # TODO: how dare you?!
...
def legal_speed_limit(v1, v2, c=None):
    '''
    ...
    v2_in_front = behind(v1, v2, c) and same_lane(v1, v2, c)
    dist = longitudinal_distance(v1, v2, c)
    safe_dist = safe_long_dist(v1, v2, c)
    vel_diff = abs(v1[3] - c["HARD_SPEED_LIMIT"])
    return v2_in_front, dist, safe_dist, vel_diff


def legal_overtake(v1, v2, c=None):
    '''
    ...
    '''
    v2_on_left = left_lane(v2, v1, c) and in_vicinity(v1, v2, c)
    vel_diff = v2[3] - v1[3]
